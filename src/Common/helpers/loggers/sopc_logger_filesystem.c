 ...
 * under the License.
 */
#include "sopc_logger_filesystem.h"
 ...
 */
static void SOPC_Logger_FileSystem_Initialize(void);

/**
 * \brief Creates a new log file and log instance and prints the starting timestamp
 */
static SOPC_Log_Instance* SOPC_Logger_FileSystem_CreateInstance(SOPC_Log_Configuration *log_configuration);
 ...
 */
static SOPC_Log_Instance* SOPC_Logger_FileSystem_CreateInstanceAssociation(SOPC_Log_Instance* pLogInst, const char* category);
 ...
 */
static bool SOPC_Logger_FileSystem_SetLogLevel(SOPC_Log_Instance* pLogInst, SOPC_Log_Level level);
 ...
 */
static bool SOPC_Logger_FileSystem_SetConsoleOutput(SOPC_Log_Instance* pLogInst, bool activate);
 ...
 */
static char* SOPC_Log_GetFilePathPrefix(SOPC_Log_Instance* pLogInst);
 ...
 */
static void SOPC_Log_VTrace(SOPC_Log_Instance* pLogInst, SOPC_Log_Level level, const char* format, va_list args);
 ...
 */
static void SOPC_Log_ClearInstance(SOPC_Log_Instance** ppLogInst);

/*
 * \brief Clears the logger manager: clear unique file name prefix for execution
 * */
static void SOPC_Log_Clear(void);

/* -------------------------------- */
/*          IMPLEMENTATION          */
/* -------------------------------- */

SOPC_Logger_FileSystem_Get(void)
{
    SOPC_Logger logger = {
        .initialize = SOPC_Logger_FileSystem_Initialize;
        .create_instance = SOPC_Logger_FileSystem_CreateInstance;
        .create_instance_assoc = SOPC_Logger_FileSystem_CreateInstanceAssociation;
        .vtrace = SOPC_Logger_FileSystem_VTrace;
        .clear_instance = SOPC_Logger_FileSystem_ClearInstance;
        .clear = SOPC_Logger_FileSystem_Clear;
    };
    return logger:
}
...
void SOPC_Logger_FileSystem_Initialize()
{
    SOPC_CSTRING_UNIQUE_LOG_PREFIX = SOPC_Time_GetStringOfCurrentTimeUTC(true);
    uniquePrefixSet = true;
}

static void SOPC_Logger_FileSystem_TracePrefixNoLock(SOPC_Log_Instance* pLogInst,
                                       SOPC_Log_Level level,
                                       bool withCategory,
                                       bool inhibitConsole)
{
...
static bool SOPC_Logger_FileSystem_Start(SOPC_Log_Instance* pLogInst)
{
    int res = 0;
    bool result = false;
    if (NULL != pLogInst && false == pLogInst->started)
    {
        ...
        if (pLogInst->file->pFile != NULL)
        {
            ...
            SOPC_Logger_FileSystem_TracePrefixNoLock(pLogInst, SOPC_LOG_LEVEL_INFO, true, true);
...
SOPC_Log_Instance* SOPC_Logger_FileSystem_CreateInstance(const char* logDirPath,
                                           ...
                                           uint16_t maxFiles)
{
    ...
    if (result != NULL)
    {
        ...
        SOPC_Logger_FileSystem_Start(result);
    }
    return result;
}

SOPC_Log_Instance* SOPC_Logger_FileSystem_CreateInstanceAssociation(SOPC_Log_Instance* pLogInst, const char* category)
{
    size_t category_len = 0;
    SOPC_Log_Instance* result = NULL;
...
bool SOPC_Logger_FileSystem_SetLogLevel(SOPC_Log_Instance* pLogInst, SOPC_Log_Level level)
{
    bool result = false;
    int res = 0;
    if (NULL != pLogInst && false != pLogInst->started)
...
bool SOPC_Logger_FileSystem_SetConsoleOutput(SOPC_Log_Instance* pLogInst, bool activate)
{
    int res = 0;
    bool result = false;
    if (NULL != pLogInst && false != pLogInst->started)
...
char* SOPC_Logger_FileSystem_GetFilePathPrefix(SOPC_Log_Instance* pLogInst)
{
...
static void SOPC_Logger_FileSystem_CheckFileChangeNoLock(SOPC_Log_Instance* pLogInst)
{
    ...
    if (pLogInst->file->nbBytes >= pLogInst->file->maxBytes)
    {
        if (pLogInst->file->filePath != NULL)
        {
            ...
            SOPC_Logger_FileSystem_TracePrefixNoLock(pLogInst, SOPC_LOG_LEVEL_INFO, false, true);
...
void SOPC_Logger_FileSystem_VTrace(SOPC_Log_Instance* pLogInst, SOPC_Log_Level level, const char* format, va_list args)
{
    int res = 0;
    int res2 = 0;
    if (NULL != pLogInst && false != pLogInst->started && level <= pLogInst->level)
    {
        Mutex_Lock(&pLogInst->file->fileMutex);
        // Check file open
        SOPC_Logger_FileSystem_TracePrefixNoLock(pLogInst, level, true, false);
        if (NULL != pLogInst->file->pFile)
        {
            ...
            SOPC_Logger_FileSystem_CheckFileChangeNoLock(pLogInst);
        }
        Mutex_Unlock(&pLogInst->file->fileMutex);
    }
}
...
void SOPC_Logger_FileSystem_Trace(SOPC_Log_Instance* pLogInst, SOPC_Log_Level level, const char* format, ...)
{
    va_list args;
    va_start(args, format);
    SOPC_Logger_FileSystem_VTrace(pLogInst, level, format, args);
    va_end(args);
}

// Deallocate the Log instance structure
void SOPC_Logger_FileSystem_ClearInstance(SOPC_Log_Instance** ppLogInst)
{
...
void SOPC_Logger_FileSystem_Clear(void)
{
